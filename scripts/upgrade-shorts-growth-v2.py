...
'''
    ...
    save["parameters"]["jsonBody"] = "={{ JSON.stringify({ topic: $('Extract Generated Topic').item.json.topic, hook: $('Validate Final Script').item.json.hook, canonical_key: $('Extract Generated Topic').item.json.canonical_key || null, subject_key: $('Extract Generated Topic').item.json.subject_key || null, mechanism_key: $('Extract Generated Topic').item.json.mechanism_key || null, payoff_key: $('Extract Generated Topic').item.json.payoff_key || null, strategy_arm: $('Extract Generated Topic').item.json.strategy_arm || null, policy_version: 'shorts-growth-v2' }) }}"
...
def patch_compose_payload_and_logging(w: dict) -> None:
    ...
    start_compose["parameters"]["jsonBody"] = "={{ JSON.stringify({ ...$json, policy_version: $('Validate Final Script').item.json.policy_version || 'shorts-growth-v2', outro_experiment_arm: $('Validate Final Script').item.json.outro_experiment_arm || 'no_outro', outro_line: $('Validate Final Script').item.json.outro_line || null }) }}"

    log = node_by_name(w, "Log Published Video")
    log["parameters"]["jsonBody"] = r'''={{ JSON.stringify({ video_id: ($('YouTube: Upload Draft').item.json.uploadId || $('YouTube: Upload Draft').item.json.id), published_at: new Date().toISOString(), policy_version: $('Validate Final Script').item.json.policy_version || 'shorts-growth-v2', topic: $('Extract Generated Topic').item.json.topic, canonical_key: $('Extract Generated Topic').item.json.canonical_key || null, topic_strategy_arm: $('Extract Generated Topic').item.json.strategy_arm || null, topic_predicted_score: $('Extract Generated Topic').item.json.score ?? null, hook: $('Validate Final Script').item.json.hook, title: $('Validate Final Script').item.json.title, comment_hook: $('Validate Final Script').item.json.comment_hook || null, caption_style: $('Validate Final Script').item.json.caption_style, trigger: $('Validate Final Script').item.json.trigger, outro_experiment_arm: $('Validate Final Script').item.json.outro_experiment_arm || null, creative_dna: { policy_version: $('Validate Final Script').item.json.policy_version || 'shorts-growth-v2', topic_strategy_arm: $('Extract Generated Topic').item.json.strategy_arm || null, topic_predicted_score: $('Extract Generated Topic').item.json.score ?? null, canonical_key: $('Extract Generated Topic').item.json.canonical_key || null, concept_archetype: $('Extract Generated Topic').item.json.archetype || null, creative_format: $('Validate Final Script').item.json.creative_format || null, visual_grammar: $('Validate Final Script').item.json.visual_grammar || null, first_frame_type: $('Validate Final Script').item.json.first_frame_type || null, first_frame_source: (($('Validate Final Script').item.json.scenes || []).find(s => !s?.template_data?.is_outro)?.visual_source || null), first_frame_score: $('Validate Final Script').item.json.quality?.first_frame_strength ?? null, scene_count: ($('Validate Final Script').item.json.scenes || []).filter(s => !s?.template_data?.is_outro).length, word_count: ($('Validate Final Script').item.json.scenes || []).filter(s => !s?.template_data?.is_outro).reduce((n,s)=>n+String(s.narration||'').trim().split(/\s+/).filter(Boolean).length,0), duration_sec: $('Validate Compose Result').item.json.duration_sec ?? null, target_duration_band: $('Validate Final Script').item.json.target_duration_band || '28-36s', length_exception_reason: $('Validate Final Script').item.json.length_exception_reason || null, payoff_position_pct: (() => { const ss=($('Validate Final Script').item.json.scenes || []).filter(s => !s?.template_data?.is_outro); const ris=Number($('Validate Final Script').item.json.payoff?.resolved_in_scene); const idx=ss.findIndex(s => Number(s.scene_index) === ris); return idx >= 0 && ss.length ? Number(((idx + 1) / ss.length).toFixed(3)) : null; })(), open_loop_count: $('Validate Final Script').item.json.open_loop_count ?? null, template_count: ($('Validate Final Script').item.json.scenes || []).filter(s => !s?.template_data?.is_outro && s.visual_source === 'template').length, real_video_count: ($('Merge By scene_index (not position)').item.json.data || []).filter(s => !s?.template_data?.is_outro && Boolean(s.video_url)).length, still_image_count: ($('Merge By scene_index (not position)').item.json.data || []).filter(s => !s?.template_data?.is_outro && Array.isArray(s.images) && s.images.length).length, caption_mode: $('Validate Final Script').item.json.caption_mode || null, transition_style: $('Validate Final Script').item.json.transition_style || 'hard_cut', engagement_mode: $('Validate Final Script').item.json.engagement_mode || null, comment_hook_present: Boolean($('Validate Final Script').item.json.comment_hook), outro_present: $('Validate Final Script').item.json.outro_experiment_arm === 'current_outro', outro_experiment_arm: $('Validate Final Script').item.json.outro_experiment_arm || null, asset_quality_min: null, asset_quality_avg: null } }) }}'''
